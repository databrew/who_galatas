...
The `cmrshp` object now contains the "close" and "far" populations per each of the 189 districts ("close" meaning within 5 km of any health facility in any district, and "far" meaning not within 5km of a health facility). 
...
Note that the `inaccuracy` column shows the difference between the _total_ population for the district, and the sum of the `close` and `far` populations. These are very minor discrepancies occurring in only very few districts. These inaccuracies are likely due to issues with border handling (did not examine in depth).
...
To double-check overall sums, one compare the overall district-level populations from the raster...

```{r}
sum(populations)
```

... with the total of the close and far populations per our calculations (26.3 million):
